...
function TablePage() {
  ...
  useEffect(() => {
    ...
    if (savedData) {
      ...
      // Fix duplicate IDs
      const uniqueEntries = entries.reduce((acc, entry, index) => {
        const existing = acc.find(e => e.id === entry.id);
        if (!existing) {
          return [...acc, entry];
        } else {
          return [...acc, { ...entry, id: Math.max(...acc.map(e => e.id)) + 1 }];
        }
      }, []);
      setTableEntries(uniqueEntries);
      localStorage.setItem('tableEntries', JSON.stringify(uniqueEntries));
    }
  }, []);
