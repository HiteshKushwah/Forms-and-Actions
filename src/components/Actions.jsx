...
function Actions() {
  ...
  const handleFormSubmit = (event) => {
    ...
    //this work after form submit

    let updatedEntries;
    if (editingId) {
      updatedEntries = tableEntries.map((entry) =>
        entry.id === editingId ? { ...formData, id: editingId } : entry
      );
      toast.success('Bhai data successfully update ho gaya!');
      setEditingId(null);
    } else {
      const newEntry = { ...formData, id: tableEntries.length + 1 };
      updatedEntries = [...tableEntries, newEntry];
  ...
  const handleEdit = (id) => {
    ...
    setFormData(entry);
    setEditingId(id);
    toast.info('Bhai, edit mode me hai, form me data load ho gaya!');
  };
  ...
  const handleDelete = (id) => {
    if (window.confirm('Bhai, sure hai delete karna hai?')) {
      ...
      setTableEntries(updatedEntries);
      localStorage.setItem('tableEntries', JSON.stringify(updatedEntries));
      toast.success('Bhai, data delete ho gaya!');
    }
  };
  ...
  return (
    <div style={{ margin: '0 auto', maxWidth: '1200px' }}>
      ...
      >
        <form onSubmit={handleFormSubmit}>
          <div style={{ marginBottom: '15px' }}>
            <label style={{ fontFamily: 'fantasy', textDecoration: 'underline',  color:"black"}}>
              First Name
            </label>
            <br />
            <input
          ...
          <div style={{ display: 'flex', gap: '15px' }}>
            <button
              ...
              style={{
                padding: '10px 20px',
                backgroundColor: editingId ? 'blue' : 'tomato',
                color: 'white',
                border: 'none',
                borderRadius: '6px',
                cursor: 'pointer',
      ...
      >
        ...
        >
          Your All Submitted Data Is Here keep It Copy Bro
        </h2>
            ...
            <tbody>
              {tableEntries.map((entry) => (
                <tr key={entry.id}>
                  ...
                  <td style={{ padding: '10px', border: 'lightgray' }}>{entry.firstName}</td>
                  <td style={{ padding: '10px', border: 'lightgray' }}>{entry.lastName}</td>
                  <td style={{ padding: '10px', border: 'lightgray' }}>{entry.email}</td>
                  <td style={{ padding: '10px', border: 'lightgray' }}>{entry.gender}</td>
                  <td style={{ padding: '10px', border: 'lightgray' }}>
                    {entry.hobbies.join(', ')}
                  </td>
                  <td style={{ padding: '10px', border: 'lightgray' }}>{entry.status}</td>
                  <td style={{ padding: '10px', border: 'lightgray' }}>{entry.description}</td>
                  <td style={{ padding: '10px', border: 'lightgray' }}>
                    {editingId === entry.id ? (
                      <button
